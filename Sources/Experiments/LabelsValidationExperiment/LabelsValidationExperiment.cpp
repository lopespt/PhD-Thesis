...
LabelsValidationExperiment::LabelsValidationExperiment(QString sunDatabaseFolder, QString complexNetworkFile)
        : sunDatabaseFolder(sunDatabaseFolder), complexNetworkFile(complexNetworkFile) {
}
    ...
    for (int teste = 0; teste < 1; teste++) {
        ...
        while (reader.hasNext()) {
            ...
            if (img.getRegions().size() > 1) {
                ...
                total++;
                printf("Acertos: %d | Analisadas: %d | Total: %d | %.0f%%\r", acertos, total + 1, reader.getTotal(),
                       acertos * 100. / total);
                fflush(stdout);
            }
        }
