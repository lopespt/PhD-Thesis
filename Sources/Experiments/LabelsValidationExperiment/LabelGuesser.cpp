...
void LabelGuesser::removeHubs() {
    ...
    qSort(degrees.begin(), degrees.end(),
          [&](QPair <node_id, float> a, QPair <node_id, float> b) { return a.second > b.second; });
...
bool LabelGuesser::Guess(SupervisedImage *img, int guessRegionAt) {
    ...
    shared_ptr<FeatureAbstract> orientation = orientation_factory.CreateFromRegion(&img->getRegions()[guessRegionAt]);
    node_id node_orientation = cn->getNodeFromFeature(orientation.get());
    ...
    for (auto i = rank.begin(); i != rank.end(); i++) {
        if (strcmp((cn->getNode(i->first))->get()->asString(buffer),
                   img->getRegions().at(guessRegionAt).getLabel().toStdString().c_str()) == 0) {
            //printf("%d \n", pos);
            break;
        }
        pos++;
